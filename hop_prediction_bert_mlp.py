...
# BertWithMLPHead class
class BertWithMLPHead(BertPreTrainedModel):
    """
    Corrected custom BERT model that inherits from BertPreTrainedModel
    to support saving and loading with .save_pretrained() and .from_pretrained().
    """
    def __init__(self, config, hidden_dim: int = 256, dropout_prob: float = 0.1):
        ...
        # Define the custom MLP classification head
        bert_output_dim = config.hidden_size # This is 768 for bert-base-uncased
        self.classifier = nn.Sequential(
            nn.Dropout(dropout_prob), # Dropout on the CLS embedding is common
        ...
        # This initializes the weights of the new layers
        # and correctly ties the model to the parent class
        self.post_init()

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None, **kwargs):
        #Get the output from the base BERT model
        outputs = self.bert(
            input_ids=input_ids,
        ...
        # We use the output of the [CLS] token for classification (pooler_output)
        cls_output = outputs.pooler_output
        
        #Pass the [CLS] output through our custom MLP head
        ...
        # Return the output in a format compatible with Hugging Face
...
# model with MLP head initialization
model = BertWithMLPHead.from_pretrained(MODEL_NAME, config=config)
model.to(DEVICE)
